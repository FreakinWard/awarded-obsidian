class AwardedTablesDailyNote {
  createSummaryText(page) {
    const emptySummary = "`-`";

    const isPerson = page.type === "person";
    const personRole = isPerson ? page.role : emptySummary;

    return isPerson ? personRole : page.summary;
  }

  createCardRowWithSummary(page) {
    const summary = this.createSummaryText(page);
  ...
  createCardRowWithSummaryMemories(page) {
    ...
    return [page.file.link, summary, timeText];
  }
  ...
  meetingsForToday(dv) {
    ...
    dv.table(
      ["File", "Summary", ""],
      dv
        .pages('"Timestamps/Meetings" OR "Extras/People"')
        .where((page) => this.isToday(page.date, journalDate))
        // .where((page) => page.file.path.startsWith("Timestamps/Meetings"))
        // .where((page) => !excludedTypes.includes(page.type))
        .sort((page) => page.file.date, "desc")
        .map((page) => this.createCardRowWithSummary(page)),
    );
  }
}
